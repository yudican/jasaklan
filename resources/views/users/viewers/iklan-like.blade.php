<x-app-layout>
    ...
    <div class="md:mx-8 mt-12 md:px-4 md:text-base text-[10px] mx-2 px-2 py-2 rounded border border-gray-500">
        ...
        <ul class="list-decimal px-8 mt-4 text-justify">
            <li>Setiap 1 like member akan di bayar {{ \App\Models\Ads::LIKE_COM }}</li>
            <li>Dana akan masuk ke menu penghasilan setelah Advertiser melakukan pengecekan maksimal 7 hari.</li>
            <li>Member dilarang keras Unlike. Bila hal ini dilakukan maka akun akan dibanned dan tidak diperbolehkan
        ...
        <div class="bg-white rounded-lg border border-gray-200 px-4 py-2 border-b my-2 rounded-t-lg flex justify-between">
            <div class="py-2">
                ...
                <div href="#" aria-current="true">
                    ...
                    <div class="inline-block">{{ $like->getCommissionFee() }}</div>
                </div>
            </div>
            <div>
                <div class="flex gap-4 py-4">
                    <div>
                        <span class="bg-red-100 text-red-800 text-xs md:text-sm font-semibold mr-2 px-2.5 py-0.5 rounded dark:bg-red-200 dark:text-red-900">{{ $like->getSocialMedia() }}</span>
                        <div>
                            <span class="text-xs">Sisa Tiket: {{ $like->getTicket() }}</span>
                        </div>
                    </div>
                    <div class="my-auto">
                        @if($like->alreadyGetTicket($like->id))
                        ...
                        @else
                        <a href="{{ route('viewers.ticket.session', $like->id) }}" class="text-white px-3 py-2 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                            Ambil Tiket
                        </a>
                        @endif
                    </div>
    ...
    @include('users.viewers.footer.content', ['title' => 'Like'])
    @include('users.viewers.modals.like', ['content' => session('content')])
